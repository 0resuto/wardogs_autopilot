...
def _frames(out_dir, newest_first=False):
    fs = [
        p
        for p in glob.glob(os.path.join(out_dir, "debug_fail_*.png"))
...
def main():
    ap = argparse.ArgumentParser(
        prog="replay_fails", description="Replay output/debug_fail_*.png frames and triage them."
    )
    ap.add_argument("--dir", default=os.path.join(ROOT, "output"))
    ap.add_argument("--map", default=mmd._map_name())
    ap.add_argument(
        "--no-prev", action="store_true", help="cold start (ignore the saved prev position)"
    )
    ap.add_argument(
        "--limit", type=int, default=0, help="process at most N newest frames (0 = all)"
    )
    ap.add_argument(
        "--budget",
        type=float,
    ...
    ap.add_argument("--json", action="store_true", help="write output/fail_report_<ts>.json")
    a = ap.parse_args()
    ...
    print(
        "map=%s  frames=%d  prev=%s  budget=%s"
        % (a.map, len(frames), "off" if a.no_prev else "from sidecar", a.budget)
    )
    ...
    for i, path in enumerate(frames, 1):
        ...
        if pose is not None:
            n_ok += 1
            place = "OK  x=%.0f y=%.0f inl=%d" % (pose["map_x"], pose["map_y"], pose["inl"])
        else:
        ...
        print(
            "  [%02d] %-38s %-28s kp=%-5s %6.0fms"
            % (i, os.path.basename(path), place, diag.get("kp_mm"), dt * 1000)
        )

    print(
        "\n%d/%d frames localize now (%.0f%%), %d still fail"
        % (n_ok, len(rows), 100.0 * n_ok / max(len(rows), 1), len(rows) - n_ok)
    )
    tally = {}
    for r in rows:
        if not r["ok"]:
    ...
    if tally:
        print("remaining failures: %s" % ", ".join("%s=%d" % kv for kv in sorted(tally.items())))

    if a.json:
        out = os.path.join(a.dir, "fail_report_%s.json" % time.strftime("%Y%m%d_%H%M%S"))
