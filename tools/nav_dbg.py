...
def bursts_of(rows):
    ...
    while i < n:
        if rows[i]["steer"] != 0:
            ...
            w = []
            j = end + 1
            while j < n and rows[j]["steer"] == 0 and \
                    rows[j]["t"] - rows[end]["t"] <= 1.2:
                w.append(rows[j])
                j += 1
            bursts.append((b, w))
        else:
...
def main():
    ...
    ap.add_argument("path", nargs="?", default=None,
                    help="log file; default: newest one in output/")
    ap.add_argument("--tail", type=int, default=0,
                    help="print last N ticks as a table")
    ap.add_argument("--bursts", type=int, default=0,
                    help="print last N steering corrections (0=none)")
    ap.add_argument("--no-bursts", action="store_true",
                    help="do not count corrections")
    args = ap.parse_args()

    path = args.path
    if not path:
        files = sorted(glob.glob(os.path.join("output", "nav_dbg_*.jsonl")),
    ...
    print("# ticks=%d  duration=%.1f s  (~%.0f ticks/s)"
    ...
    pa = stats(pose_age)
    sa = stats(sample_age)
    print("pose age pose_age:       mean=%.2f  p95=%.2f  max=%.2f s"
          % pa)
    print("sample age sample_age:   mean=%.2f  p95=%.2f  max=%.2f s"
          % sa)
    lost = [e for e in events if e.get("kind") == "lost"]
    if lost:
        print("localization losses (pose jump rejected): %d times"
              % len(lost))
        for e in lost[:12]:
            print("    t=%5.2f  pause=%s  offset~%.1fm  claimed speed="
                  "%.0f px/s" % (e["t"] - rows[0]["t"], e.get("sa"),
                                  e.get("d", 0.0), e.get("v_claim", 0.0)))
        if len(lost) > 12:
            print("    ... and %d more" % (len(lost) - 12))
    if rows[0].get("mh_age") is not None:
        ma = stats([r["mh_age"] for r in rows
                    if r.get("mh_age") is not None])
        print("M-course age mh_age:    mean=%.2f  p95=%.2f  max=%.2f s"
              % ma)
    ...
    ms = "  ".join("%s=%d (%.0f%%)" % (k, v, pct(v, len(rows)))
                   for k, v in sorted(modes.items()))
    print("course source (th): %s" % ms)
    print("steering held: %d ticks (%.0f%%), avg hold duration=%.2f s"
          % (steer_ticks, pct(steer_ticks, len(rows)),
             sum(hold_all) / max(len(hold_all), 1)))
    ...
    if not args.no_bursts:
        ...
        for _b, w in bursts:
            if w:
                over.append(max(min(r["err"] for r in w),
                                -min(r["err"] for r in w),
                                key=abs))
        o = stats(over) if over else (0.0, 0.0, 0.0)
        print("steering corrections: %d" % len(b_all))
        print("overshoot in release window (|err|): mean=%.1f p95=%.1f "
              "max=%.1f deg" % o)
        ...
        for b in b_all:
            ...
            held += len(b)
        print("holds during which a new sample arrived: %d/%d "
              "(%.0f%%)" % (got, len(b_all), pct(got, len(b_all))))
        if has_fresh:
            print("blind hold (no fresh sample): %d ticks out of %d "
                  "(%.0f%%)" % (blind, held, pct(blind, held)))
        else:
            print("blind hold (pose_age>0.5 s): %d ticks out of %d (%.0f%%)"
                  % (blind, held, pct(blind, held)))

        if args.bursts:
            print("\n# last %d corrections:" % args.bursts)
            print("%6s %5s %5s %5s %5s %6s %6s %6s %6s %7s %6s %5s %5s %5s %4s"
                  % ("t,s", "err?", "err!", "hold", "imp", "dHead", "extrERR",
                     "dErr?", "fresh", "poseAge", "sampA", "mode", "keys",
                     "settle", "new"))
            n = len(b_all)
            for bi in range(max(0, n - args.bursts), n):
                ...
                fresh_n = sum(1 for r in b if r.get("fresh", True)) if has_fresh \
                    else None
                pa_m = max(r["pose_age"] for r in b)
                sa_m = max(r["sample_age"] for r in b)
                new = sum(1 for r in b if r["new_sample"])
                print("%6.2f %5.1f %5.1f %5.2f %5.2f %6.1f %6.1f %6.1f %6s "
                      "%7.2f %6.2f %5s %5s %5.2f %4d"
                      % (b[0]["t"] - rows[0]["t"], e0, e1, h, imp, dh, ext, dd,
                         "%d/%d" % (fresh_n, len(b)) if fresh_n is not None
                         else "-",
                         pa_m, sa_m, b[0]["mode"], b[0]["keys"],
                         max(r.get("settle", 0.0) for r in w) if w else 0.0,
                         new))

    if args.tail:
        print("\n# last %d ticks (err column: deviation, + steer D, - steer A,"
              " . neutral, * new sample, STALE if pose older than 0.5 s)"
              % args.tail)
        print("%6s %5s %5s %6s %5s %5s %6s %6s %5s %7s %6s %5s %s %s"
              % ("t,s", "err", "head", "steer", "hold", "v", "km/h",
                 "tgt", "xte", "settle", "poseA", "mode", "keys", "ru"))
        for r in rows[-args.tail:]:
            ...
            print("%6.2f %5.1f %5.1f %6s %5.2f %5.0f %5.0f %6.0f %5s %6.2f "
                  "%7.2f %5s %s%s %s"
                  % (r["t"] - rows[0]["t"], r["err"], r["heading"], st,
                     r.get("hold", 0.0), r.get("v", 0.0),
                     r.get("vkmh", 0.0), r.get("tgt", 0.0),
                     "--" if r.get("xte_m") is None
                     else "%.1fm" % r["xte_m"],
                     r.get("settle", 0.0),
                     r["pose_age"], r["mode"], r["keys"], flag, ru))
    return 0
