...
def main():
    ...
    ap.add_argument(
        "--conv", choices=CONVS, default=None, help="color->gray conversion (default: keep current)"
    )
    ap.add_argument(
        "--gamma",
        type=float,
    ...
    ap.add_argument(
        "--no-index", action="store_true", help="rebuild map caches only, not the feature index"
    )
    ap.add_argument("maps", nargs="*", help="map names to rebuild (default: config map.name)")
    args = ap.parse_args()

    cfg = load_config()
    mcfg = cfg.setdefault("map", {})
    old_sig = "%s-%.3f" % (mcfg.get("gray_conv", "luma"), float(mcfg.get("gray_gamma", 1.0)))
    if args.conv:
    ...
    for name in names:
        ...
        print(
            "  map %dx%d (%.1fs)  on-disk gray=%s %s"
            % (mu.shape[0], mu.shape[1], time.time() - t0, disk_sig, "OK" if ok else "MISMATCH")
        )
        if not ok:
            print(
                "  ABORT: mu cache does not match config gray — index "
        ...
        if not args.no_index:
            ...
            print(
                "  feature index rebuilt: %s (%.1fs)" % (os.path.basename(path), time.time() - t1)
            )
