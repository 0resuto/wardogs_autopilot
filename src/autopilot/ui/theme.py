...
_RESET_OPTS = (
    "background", "foreground", "troughcolor", "bordercolor", "darkcolor",
    "lightcolor", "focuscolor", "selectbackground", "selectforeground",
    "fieldbackground", "insertcolor", "sliderlength", "arrowcolor", "padding",
)
...
class ThemeManager:
    ...
    def apply(self) -> None:
        ...
        if not dark:
            s.theme_use("vista")
            for opts in (".", "TFrame", "TLabel", "TButton", "TEntry", "TSpinbox",
                         "TCombobox",
                         "Horizontal.TScale", "Vertical.TScrollbar", "Horizontal.TScrollbar",
        ...
        s.configure(".", background=bg, foreground=fg, troughcolor="#1a1a1a",
                    bordercolor=hi, darkcolor=bg, lightcolor=hi, focuscolor="#1b6ac9",
                    selectbackground="#1b6ac9", selectforeground=fg)
        s.configure("TFrame", background=bg)
        s.configure("TLabel", background=bg, foreground=fg)
        s.configure("TNotebook", background=bg, borderwidth=0)
        s.configure("TNotebook.Tab", background=panel, foreground=fg, padding=(10, 4), borderwidth=0)
        s.map("TNotebook.Tab", background=[("selected", hi)], foreground=[("selected", "#ffffff")])
        s.configure("TButton", background=hi, foreground=fg, bordercolor=hi, padding=(8, 3), focuscolor=hi)
        s.map("TButton", background=[("active", "#4e4e4e"), ("pressed", "#333333")],
              foreground=[("active", fg)])
        s.configure("TEntry", fieldbackground=panel, foreground=fg, bordercolor=hi, insertcolor=fg)
        s.configure("TSpinbox", fieldbackground=panel, foreground=fg, bordercolor=hi, insertcolor=fg)
        s.configure("TCombobox", fieldbackground=panel, foreground=fg, bordercolor=hi,
                    background=hi, arrowcolor=fg, insertcolor=fg)
        ...
        s.configure("Vertical.TScrollbar", background=hi, troughcolor=bg, bordercolor=bg, arrowcolor=fg)
        s.configure("Horizontal.TScrollbar", background=hi, troughcolor=bg, bordercolor=bg, arrowcolor=fg)
