        ...
        try:
            while not self._stop.is_set():
                try:
                    ...
                    ui = self.mask
                    if ui.shape[:2] != (h, w):
                        ui = cv2.resize(ui.astype(np.uint8), (w, h),
                                        interpolation=cv2.INTER_NEAREST) > 0
                    item = dict(ts=time.time(), gray=mm, bgr=frame, roi=roi,
                                mask=ui)
                    try:
...
class LiveLocator(threading.Thread):
    ...
    def __init__(self, cfg: AppConfig | dict, mask, frame_source=None) -> None:
        ...
        self._vote_buf: collections.deque[tuple[Any, ...]] = collections.deque(maxlen=self._loc_vote_frames())
    ...
    def locator_cfg(self) -> LocatorConfig:
        if isinstance(self.cfg, dict) and "locator" in self.cfg and isinstance(self.cfg["locator"], dict):
            try:
    ...
    def run(self) -> None:
        try:
            ...
            prod = _CaptureProducer(self.cfg, self.mask, self.frame_source,
                                    self._stop, q)
            prod.start()
            while not self._stop.is_set():
                try:
                ...
                pose, diag = locator.global_pose(
                    mm, self.mask, prev_xy=self._prev_xy,
                    prev_th=self._prev_th, debug=True, budget=3.0,
                ...
                if slow > 3.0 and self.attempt % 20 == 0:
                    logger.warning(
                        "[locator] frame took %.1f s (budget 3 s) — map search", slow
                    )
                ...
                if cand is not None:
                    ...
                    if self._last_accepted is not None:
                        d_jump = math.hypot(cand[0] - self._last_accepted[0],
                                            cand[1] - self._last_accepted[1])
                        ...
                        if why is not None and cand[3] < inl_skip:
                            vote = _vote_decide(self._vote_buf, need, rad,
                                                (cand[0], cand[1]))
                            if vote is None:
                                diag['reject'] = 'vote_reject'
                                diag['detail'] = ('%s needs %d agreeing '
                                                  'frames (have %d)'
                                                  % (why, need,
                                                     len(self._vote_buf)))
                                diag['vote'] = dict(count=len(self._vote_buf),
                                                    need=need, dist=d_jump,
                                                    th=d_th, accepted=False,
                                                    inl=cand[3])
                                self.latest = dict(ts=t0, pose=None,
                                                   map_px=None, good=False,
                                                   elapsed=elapsed, diag=diag)
                                self.attempt += 1
                                continue
                            self._vote_buf.clear()
                            diag['vote'] = dict(count=len(vote), need=need,
                                                dist=d_jump, th=d_th,
                                                accepted=True, inl=cand[3])
                ...
                if pose is None:
                    ...
                    if hf > 0 and self._good_xy is not None:
                        ...
                        if self._hold_left > 0:
                            ...
                            d2 = dict(diag)
                            d2['reject'] = 'hold'
                            d2['detail'] = ('holding last pose '
                                            '(%d left)' % self._hold_left)
                            self.latest = dict(ts=t0, pose=self._good_pose,
                                               map_px=self._good_xy,
                                               map_px_disp=self._good_xy,
                                               good=False, elapsed=elapsed,
                                               diag=d2)
                            self.attempt += 1
                            continue
                    ...
                    self._hold_left = None
                self.latest = dict(ts=t0, pose=pose, map_px=mp,
                                   map_px_disp=disp, good=good,
                                   elapsed=elapsed, diag=diag)
                self.attempt += 1
        except Exception as exc:  # noqa: BLE001
